...
export class AnnouncementUpdateRequest implements AsyncTask {
	...
	constructor(guildId: string, ann: Announcement, callback: TaskCallback) {
    ...
    execute(): void {
        let bodyRaw: any = {
            ...
            "announcement_id": this.announcement.announcementId,
        ...
        $.ajax({
            url: this.apiUrl + "/v2/announcement/update",
            headers: {
            ...
            success: function (json: any) {
                let status = new NetworkCallStatus(true, TaskType.ANNOUNCEMENT_UPDATE);
                status.code = 200;
                status.body = json;
            ...
            error: function (jqXHR: jqXHR) {
                let status = new NetworkCallStatus(false, TaskType.ANNOUNCEMENT_UPDATE);

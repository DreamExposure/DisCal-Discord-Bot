...
export class CalendarGetRequest implements AsyncTask {
    ...
    execute(): void {
        ...
        $.ajax({
            url: this.apiUrl + "/v2/calendar/get",
            headers: {
            ...
            success: function (json: any) {
                let status = new NetworkCallStatus(true, TaskType.CALENDAR_GET);
                status.code = 200;
                status.body = json;
            ...
            error: function (jqXHR: jqXHR) {
                let status = new NetworkCallStatus(false, TaskType.CALENDAR_GET);

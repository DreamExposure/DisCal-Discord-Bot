...
export class EventCreateRequest implements AsyncTask {
    ...
    execute(): void {
        ...
        $.ajax({
            url: this.apiUrl + "/v2/events/create",
            headers: {
            ...
            success: function (json: any) {
                let status = new NetworkCallStatus(true, TaskType.EVENT_CREATE);
                status.code = 200;
                status.body = json;
            ...
            error: function (jqXHR: jqXHR) {
                let status = new NetworkCallStatus(false, TaskType.EVENT_CREATE);

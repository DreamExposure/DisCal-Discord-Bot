...
@Component
class EventRecurrenceYearlyVariableModal(
    private val permissionService: PermissionService,
    private val calendarService: CalendarService,
    private val embedService: EmbedService,
    private val componentService: ComponentService,
): InteractionHandler<ModalSubmitInteractionEvent> {
    override val ids = arrayOf("event-wizard.recurrence.yearly_variable")
    ...
    override suspend fun handle(event: ModalSubmitInteractionEvent, settings: GuildSettings) {
        ...
        val selectedMonth = inputs.first { it.customId == "select.event.recurrence.month" }
        ...
        // Apply changes and send response
        ...
        event.createFollowup(getCommonMsg("modal.event.recurrence.yearly.success.variable", settings.locale, selectedPosition.name, selectedDay.first().dayOfWeek.name, selectedMonth.name))
